...
function Tabs<T extends string = string>({
  ...
  return (
    ...
    >
      <div className={cn("flex flex-col gap-2", className)} data-slot="tabs" {...(props as React.ComponentProps<"div">)}>
        {children}
      </div>
    </TabsContext.Provider>
  )
...
function TabsTrigger({ ref, value, children, className, ...props }: TabsTriggerProps) {
  ...
  React.useImperativeHandle(ref as unknown as React.Ref<HTMLButtonElement>, () => localRef.current as HTMLButtonElement)
  ...
  return (
    <MotionHighlightItem className="size-full" value={value}>
      <motion.button
        className={cn(
          "inline-flex cursor-pointer items-center size-full justify-center whitespace-nowrap rounded-sm px-4 py-2 text-sm font-medium ring-offset-background transition-transform focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 data-[state=active]:text-foreground z-[1]",
          className,
        )}
...
function TabsContents({
  ...
  return (
    <div className={cn("overflow-hidden", className)} data-slot="tabs-contents" {...(props as React.ComponentProps<"div">)}>
      <motion.div
        animate={{ x: `${activeIndex * -100}%` }}
        className="flex h-full -mx-2"
        transition={transition}
      >
        {childrenArray.map((child, index) => (
          <div className="h-full w-full shrink-0 px-2" key={index}>
            {child}
          </div>
        ))}
      </motion.div>
