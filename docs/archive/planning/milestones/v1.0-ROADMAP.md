...
- [x] 04-06-page-banner-verify-PLAN.md — page.tsx server-component + banner + Phase 4 verification gate (verified-with-deferred-smoke; see footnote on Phase 4 line above)
...
- [x] 06-02-PLAN.md — Prisma schema (Conversation, SavedProduct, VisitorCustomerLink) + partial unique indexes + [BLOCKING] migration push
- [x] 06-03-PLAN.md — shopify.app.toml [app_proxy] block (STR-03)
- [x] 06-04-PLAN.md — withAppProxyHmac wrapper + lib/rate-limit/memory.ts sliding-window limiter
- [x] 06-05-PLAN.md — esbuild install ([ASSUMED] gate) + scripts/build-storefront-bundle.ts + package.json prebuild
- [x] 06-06-PLAN.md — lib/identity/merge.ts (D-11 transactional merge helper)
- [x] 06-07-PLAN.md — REST conversations routes (GET list / POST create / GET id / PATCH append / DELETE bulk)
- [x] 06-08-PLAN.md — REST saved-products routes (GET / POST toggle / DELETE single) with ON CONFLICT DO NOTHING
- [x] 06-09-PLAN.md — /api/proxy/chat real implementation replacing 501 stub (D-21 + IDN-02 cross-check + D-19 onFinish)
- [x] 06-10-PLAN.md — Inngest retention-sweep cron + register + manual cleanup-conversations script
...
- [x] 08-01-PLAN.md — Wave 0 RED test scaffolds (11 mandatory + 1 optional gated integration test)
...
- [x] 08.1-03-PLAN.md — StorefrontDrawer composition: DrawerBody + ChatPane/HistoryPanel/SavedProductsPanel from chat-ui (B-3)
...
- [x] 08.1-06-PLAN.md — Verification gate: REQUIREMENTS.md back-mark + Nyquist VALIDATION.md flips + 08.1-VERIFICATION.md + re-run /gsd-audit-milestone (H-1, H-2, D-17)
