...
- **D-12:** Phase 4 IN-01 (deferred) — the hex literals `#008060`, `#e1e3e5`, `#6d7175`, `#202223` in `chat-shell.tsx` are ALREADY arbitrary-value Tailwind classes (`bg-[#008060]`, `text-[#008060]`, `border-[#e1e3e5]`, `text-[#6d7175]`, `text-[#202223]`) — they are valid Tailwind 4 arbitrary-value class syntax, not raw CSS. The D-12 cleanup is therefore a **verify-only no-op**: confirm via grep that no bare hex literals remain OUTSIDE arbitrary-value class syntax (e.g., no inline `style={{ color: "#008060" }}`, no CSS custom properties with raw hex values). No code-level change is required — the existing arbitrary-value classes are preserved byte-identically during the surface-shell rebuild. If a future iteration wants to map these to named Tailwind tokens (e.g., `bg-emerald-600`), that is out of Phase 5 scope.
